...
__2024年6月9日　男子中フェスタ　授業体験（城北中）__
...
function draw_rect(A::Matrix , n=6 , k=0)
    ...
    for i = 1:n ,j = 1:n
        if mod(A[i,j],2) == 0
            plot!(rect_number(i,j),fill=true,color="#74F9FD",label=false,alpha=.7)         #1
        else
            plot!(rect_number(i,j),fill=true,color="#72F64A",label=false,alpha=.7)      #2
        end
     end
    plot!(title = k)
end
